...
function getCongestionGrade(value) {
  if (value < 40) {
    return { label: '여유', color: '#2fb344' }
  }
  if (value < 70) {
    return { label: '보통', color: '#f2c94c' }
  }
  if (value < 90) {
    return { label: '혼잡', color: '#f2994a' }
  }
  return { label: '매우혼잡', color: '#d94343' }
}
...
export default function RouteDetail({ routeTarget, mapCenter, companyName, departureTime, onClose }) {
  ...
  const bars = steps.map((s, i) => {
    ...
    const color = s.trafficType === 3 ? WALK_COLOR : getTransitColor(s)
    return (
      <div
        key={i}
  ...
  const congestionSummary = (() => {
    const subs = steps.filter(s => s.trafficType !== 3 && s.avg_congestion != null)
    if (subs.length === 0) return null

    const totalTransitTime = subs.reduce((sum, s) => sum + (s.sectionTime || s.time || 0), 0)
    const weightedSum = subs.reduce(
      (sum, s) => sum + ((s.sectionTime || s.time || 0) * s.avg_congestion),
      0
    )
    const avgValue = totalTransitTime > 0
      ? weightedSum / totalTransitTime
      : subs.reduce((sum, s) => sum + s.avg_congestion, 0) / subs.length
    const peakValue = Math.max(...subs.map(s => s.max_congestion || 0))
    const grade = getCongestionGrade(avgValue)

    return {
      avgText: avgValue.toFixed(1),
      peakText: peakValue.toFixed(1).replace(/\.0$/, ''),
      grade,
    }
  })()
        ...
        {!loading && !error && info && (
          <>
            ...
            <div className="route-summary">
              ...
              {congestionSummary && (
                <div className="route-summary-congestion">
                  <span>· 혼잡도 {congestionSummary.avgText}% (최대 {congestionSummary.peakText}%)</span>
                  <span className="congestion-grade-chip">
                    <span
                      className="congestion-grade-dot"
                      style={{ backgroundColor: congestionSummary.grade.color }}
                    />
                    <span>{congestionSummary.grade.label}</span>
                  </span>
                </div>
              )}
            </div>
              ...
              <ul className="step-list">
                {steps.map((s, i) => {
                  ...
                  if (t === 3) {
                    ...
                    const busCongestion =
                      s.avg_congestion != null
                        ? `혼잡도 추정 평균 ${s.avg_congestion.toFixed(0)}%, 최대 ${s.max_congestion != null ? s.max_congestion.toFixed(0) : '-'}%`
                        : '혼잡도 정보 없음'
                    contentJsx = (
                      <>
                        ...
                        <div className="step-meta-small">{busCongestion}</div>
                      </>
                    )
                  }

                  const cls = t === 2 ? 'bus' : t === 3 ? 'walk' : 'subway'
                  const lineColor = t === 3 ? WALK_COLOR : getTransitColor(s)

                  return (
                    <li key={i} className={`step-item ${cls}`} style={{ color: lineColor }}>
